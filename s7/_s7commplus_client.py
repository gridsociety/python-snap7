    ...
    while offset < len(response):
        ...
        if tag == ElementID.START_OF_OBJECT:
            ...
            if attr_id == Ids.OBJECT_VARIABLE_TYPE_NAME and stack:
                # Names come as a WString. On S7-1500 the ASCII range is transmitted
                # one byte per character (no null high-bytes); genuine UTF-16 content
                # carries null bytes. Pick the decoding accordingly.
                try:
                    if b"\x00" in value:
